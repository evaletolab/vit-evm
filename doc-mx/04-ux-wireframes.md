...
| Landing `<nom>@vit.app` + vault 3 codes + restore | ✅ livré (V1.1), **E2E Sepolia à valider** |
...
- **Création** : nom `<nom>@vit.app` (landing Argent) → contact (pseudo requis) → Face ID → `/<nom>/vault` (3 codes guardians, défaut 1 coffre + 2 QR) → armement on-chain.
...
**Choix V1.1** : option (A). Le payload sauvegardé (coffre / QR) embarque l’**adresse Safe** et le nom local, donc le restore ne demande que **2 codes** — pas de saisie d’adresse. Le nom `<nom>@vit.app` reste un libellé local : sans registre on-chain (V2), il ne suffit pas à retrouver le Safe.
...
| **P8** | Nom `<nom>@vit.app` + profil émetteur à la création · joint aux envois | ✅ landing + pseudo/tél/e-mail + import contact |
...
│  appareil                   │
│                             │
│      alice@vit.app          │  ← input aligné à droite
