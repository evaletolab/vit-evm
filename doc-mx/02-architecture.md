...
**Identité** `<nom>@vit.app` (`wallet-name.ts`) : locale en V1 (pas de registre). Sert d'URL (`/<nom>/vault`, `/<nom>/restore`) et d'`id` du coffre. L'adresse Safe voyage dans la charge utile du code (`resolveWalletAddress`).
...
- Registre de noms on-chain (V2) pour unicité + payer `alice@vit.app`.
